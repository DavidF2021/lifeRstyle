...
In particular, HIS15_cleaned refers to a cleaned dataset derived from the CSO HIS15_cleaned
...
We begin by fitting a simple linear regression model to the HIS15_cleaned dataset.
The fit() function is applied with fit_type = "lm" to specify a standard
linear model.

```{}
lm_fit<-fit(HIS15_cleaned,fit_type="lm")
lm_fit
```
...
```{}
anova_fit<-fit(HIS15_cleaned,fit_type="anova")
anova_fit
```
